...
def train_model(
    ...
    for sideset in sidesets:
        ...
        my_operators.append(
            nnopinf.models.WrappedOperatorForModel(
                operator=op,
                inputs=("u-" + sideset,),
                name="BC-" + sideset + "-" + str(ensemble_id),
            )
        )

    stiffness_operator = nnopinf.models.WrappedOperatorForModel(
        operator=NpdMlp, inputs=("x",), name="stiffness-" + str(ensemble_id)
    )
    ...
    nnopinf.training.train(
        my_model,
        input_dict=inputs,
        y=reshape_snapshots(uhat_ddots).transpose(),
...
def non_parametric_fit_with_grid_search_beta(
    ...
    for counter, regularization_parameter in enumerate(
        ...
        for counter2, regularization_parameter2 in enumerate(
            ...
            for counter3, regularization_parameter3 in enumerate(
                ...
                print(
                    f"Error: {errors[counter]:.4e}, Regularization Parameter 1: {regularization_parameter:.4e}, Regularization Parameter 2: {regularization_parameter2:.4e}, Regularization Parameter 3: {regularization_parameter3:.4e}"
                )
                counter += 1
...
def non_parametric_fit_with_grid_search(
    ...
    for counter, regularization_parameter in enumerate(
        ...
        print(
            f"Error: {errors[counter]/n_cases:.4e}, Regularization Parameter: {regularization_parameter:.4e}"
        )
        counter += 1
    optimal_case = np.nanargmin(errors)
    optimal_regularization_parameter = regularization_parameters_to_try[optimal_case]
            ...
            if "Schwarz DN nonoverlap" in bc:
                ...
                for i in range(0, n_bcs):
                    ...
                    if "default BC type" not in bc_entry:
                        raise ValueError(
                            "Schwarz DN nonoverlap boundary condition for side set "
                            + name
...
def get_processed_snapshots(opinf_settings):
    ...
    for i in range(n_training_cases):
        ...
        displacement_snapshots_in, times_in = (
            normaopinf.readers.load_displacement_csv_files(
        ...
        free_dofs = normaopinf.readers.get_free_dofs(
            solution_directory=cur_dir, base_name=base_name
        )
        if i == 0:
            free_dofs_check = free_dofs.copy()
        else:
            assert np.allclose(
                free_dofs, free_dofs_check
        ...
        if opinf_settings["stop-training-time"] != "end":
            stop_index = np.argmin(
                np.abs(times_in - float(opinf_settings["stop-training-time"]))
            )
        ...
        if i == 0:
            ...
            displacement_snapshots = np.append(
                displacement_snapshots, displacement_snapshots_in, axis=2
            )
            velocity_snapshots = np.append(
                velocity_snapshots, velocity_snapshots_in, axis=2
            )
            acceleration_snapshots = np.append(
                acceleration_snapshots, acceleration_snapshots_in, axis=2
            )
    ...
    if len(disp_sidesets) > 0:
        ...
        for i in range(n_training_cases):
            ...
            sideset_snapshots_in = (
                normaopinf.readers.load_sideset_displacement_csv_files(
                    solution_directory=cur_dir,
            ...
            if opinf_settings["stop-training-time"] != "end":
                for sideset in disp_sidesets:
                    sideset_snapshots_in[sideset] = sideset_snapshots_in[sideset][
                        ..., 0:stop_index
                    ]
            ...
            if i == 0:
                ...
                for sideset in disp_sidesets:
                    sideset_snapshots[sideset] = np.append(
                        sideset_snapshots[sideset],
                        sideset_snapshots_in[sideset],
                        axis=2,
                    )
        ...
        for i in range(n_training_cases):
            ...
            if opinf_settings["stop-training-time"] != "end":
                for sideset in sideset_force_in.keys():
                    sideset_force_in[sideset] = sideset_force_in[sideset][
                        ..., 0:stop_index
                    ]
            ...
            if len(sideset_force_snapshots) == 0:
                ...
                for sideset in sideset_force_snapshots.keys():
                    if sideset not in sideset_force_in:
                        raise ValueError(
                            "Missing force snapshots for sideset " + sideset
                        )
                    sideset_force_snapshots[sideset] = np.append(
                        sideset_force_snapshots[sideset],
                        sideset_force_in[sideset],
                        axis=2,
                    )

    # Set values = 0 if DOFs are fixed
    displacement_snapshots[free_dofs[:, :] == False] = 0.0
    velocity_snapshots[free_dofs[:, :] == False] = 0.0
    acceleration_snapshots[free_dofs[:, :] == False] = 0.0
...
def convert_nodesets_to_sidesets(snapshots_dict, nodesets, sidesets):
    ...
    for i, nodeset in enumerate(nodesets):
        sideset = sidesets[i]
        data = np.append(
            snapshots_dict["sidesets"][nodeset + "-x"],
            snapshots_dict["sidesets"][nodeset + "-y"],
            axis=0,
        )
...
def build_sideset_reduced_snapshots(sideset_snapshots, opinf_settings):
    ...
    if len(sidesets) == 0:
        return (
            ...
            ss_tspace,
            ss_tspace_energy,
            reduced_stacked_sideset_snapshots,
        )

    if opinf_settings["boundary-truncation-type"] == "energy":
        my_boundary_truncater = romtools.vector_space.utils.EnergyBasedTruncater(
            opinf_settings["boundary-truncation-value"]
        )
    elif opinf_settings["boundary-truncation-type"] == "size":
        my_boundary_truncater = romtools.vector_space.utils.BasisSizeTruncater(
            opinf_settings["boundary-truncation-value"]
        )
    else:
        print(
            "Truncater " + opinf_settings["boundary-truncation-type"] + " not supported"
        )
    ...
    for sideset in sidesets:
        ...
        reshaped_snapshots = np.reshape(
            ...
            (
                snapshot_shape[0],
                snapshot_shape[1],
                snapshot_shape[2] * snapshot_shape[3],
            ),
        )
        ss_tspace_energy[sideset] = np.zeros(0)

        if (
            sideset_snapshots[sideset].shape[0] == 1
            or opinf_settings["trial-space-splitting-type"] == "combined"
        ):
            ...
            ss_tspace_energy[sideset] = np.append(
                ss_tspace_energy[sideset], my_boundary_truncater.get_energy()
            )
        else:
            comp_trial_space = []
            for i in range(0, 3):
        ...
        reduced_sideset_snapshots[sideset] = romtools.rom.optimal_l2_projection(
            reshaped_snapshots, ss_tspace[sideset]
        )
        reduced_sideset_snapshots[sideset] = np.reshape(
            reduced_sideset_snapshots[sideset],
            (
                reduced_sideset_snapshots[sideset].shape[0],
                snapshot_shape[2],
                snapshot_shape[3],
            ),
        )
    ...
    for sideset in sidesets:
        if reduced_stacked_sideset_snapshots is None:
            reduced_stacked_sideset_snapshots = reduced_sideset_snapshots[sideset] * 1.0
        else:
...
def make_opinf_model_from_snapshots_dict(snapshots_dict, opinf_settings):
    ...
    if input_scale_type == "rms":
        ...
        sideset_force_snapshots = apply_sideset_scales(
            sideset_force_snapshots, force_input_scales
        )
    ...
    (
        reduced_sideset_force_snapshots,
        ss_force_tspace,
        ss_force_tspace_energy,
        reduced_stacked_force_snapshots,
    ...
    if reduced_stacked_sideset_snapshots is None:
        reduced_stacked_sideset_snapshots = np.zeros(
            (1, displacement_snapshots.shape[-2], displacement_snapshots.shape[-1])
        )

    # Create trial space for main DOFs
    if opinf_settings["truncation-type"] == "energy":
        my_truncater = romtools.vector_space.utils.EnergyBasedTruncater(
            opinf_settings["truncation-value"]
        )
    elif opinf_settings["truncation-type"] == "size":
        my_truncater = romtools.vector_space.utils.BasisSizeTruncater(
            opinf_settings["truncation-value"]
        )
    else:
        print(
    ...
    reshaped_snapshots = np.reshape(
        ...
        (snapshot_shape[0], snapshot_shape[1], snapshot_shape[2] * snapshot_shape[3]),
    )

    tspace_energy = np.zeros(0)
    if opinf_settings["trial-space-splitting-type"] == "combined":
        trial_space = romtools.VectorSpaceFromPOD(
            snapshots=reshaped_snapshots,
    ...
    uhat_dots = np.reshape(
        uhat_dots, (uhat_dots.shape[0], snapshot_shape[2], snapshot_shape[3])
    )
    ...
    if opinf_settings["acceleration-computation-type"] == "acceleration-snapshots":
        ...
        uhat_ddots = np.reshape(
            uhat_ddots, (uhat_ddots.shape[0], snapshot_shape[2], snapshot_shape[3])
        )

    elif opinf_settings["acceleration-computation-type"] == "finite-difference":
        u_ddots = normaopinf.calculus.d2dx2(
            displacement_snapshots, times, method="2nd-order-uniform-dx"
        )
    ...
    if opinf_settings["model-type"] in opinf_polynomial_models:
        ...
        if isinstance(
            opinf_settings["regularization-parameter"], np.ndarray
        ) or isinstance(opinf_settings["regularization-parameter"], list):
            opinf_model, regularization_parameter = non_parametric_fit_with_grid_search(
                ...
                times=times,
                regularization_parameters_to_try=opinf_settings[
                    "regularization-parameter"
                ],
            )
            ...
            opinf_model.fit(
                states=uhat, ddts=uhat_ddots, inputs=reduced_stacked_sideset_snapshots
            )
        ...
        for sideset in sidesets:
            print(
                f"    Displacement sideset {sideset} basis size: {np.shape( ss_tspace[sideset].get_basis() )[-1]}"
            )
        for sideset in force_sidesets:
            print(
                f"    Force sideset {sideset} basis size: {np.shape( ss_force_tspace[sideset].get_basis() )[-1]}"
            )
        print("=========================" + RESET)
        ...
        for sideset in sidesets:
            ...
            val = np.einsum(
                "kr,vnr->vkn",
                B[:, col_start : col_start + num_dofs],
                ss_tspace[sideset].get_basis(),
            )
            shape2 = (
                B[:, col_start : col_start + num_dofs]
                @ ss_tspace[sideset].get_basis()[0].transpose()
            )
        ...
        for sideset in force_sidesets:
            ...
            val = np.einsum(
                "kr,vnr->vkn",
                B[:, col_start : col_start + num_dofs],
                ss_force_tspace[sideset].get_basis(),
            )
            shape2 = (
                B[:, col_start : col_start + num_dofs]
                @ ss_force_tspace[sideset].get_basis()[0].transpose()
            )
        ...
        for sideset in sidesets:
            ...
            if input_scale_type == "rms":
                vals_to_save["input-scale-B_" + sideset] = disp_input_scales.get(
                    sideset, 1.0
                )
            if opinf_settings.get("save-sideset-bases", False):
                vals_to_save["basis-B_" + sideset] = ss_tspace[sideset].get_basis()
        for sideset in force_sidesets:
            vals_to_save[sideset + "-force-energy-cutoff"] = ss_force_tspace_energy[
                sideset
            ]
            if input_scale_type == "rms":
                vals_to_save["input-scale-B_N_" + sideset] = force_input_scales.get(
                    sideset, 1.0
                )
            if opinf_settings.get("save-sideset-bases", False):
                vals_to_save["basis-B_N_" + sideset] = ss_force_tspace[
                    sideset
                ].get_basis()
